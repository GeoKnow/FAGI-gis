...
public class ShiftAToB extends AbstractFusionTransformation {
    private static final String ID = "ShiftAToB";  
    private static final int intID = 6;
    private float shift;
    private float scale;
    ...
    public void fuse(Connection connection, String nodeA, String nodeB) throws SQLException {
        ...
        final String queryString = "SELECT ST_X(ST_Centroid(a.geom)), ST_Y(ST_Centroid(a.geom)), ST_X(ST_Centroid(b.geom)), ST_Y(ST_Centroid(b.geom)), ST_AsText(a.geom) FROM dataset_a_geometries a, dataset_b_geometries b "
    ...
    public void fuseAll(Connection connection) throws SQLException {        
        ...
        final String insertShiftToPoint =
                ...
                + "SELECT links.nodea, links.nodeb, ST_Transform(GeomRotate(GeomResize(ST_Translate(a_g, (b_x-a_x) * (?), (b_y-a_y) * (?)), ?), ?), 4326) AS geom\n"
                + "FROM links \n"
    ...
    public void fuseCluster(Connection connection) throws SQLException {
        ...
        final String insertShiftToPoint =
                "INSERT INTO fused_geometries (subject_A, subject_B, geom) \n"
                + "SELECT cluster.nodea, cluster.nodeb, ST_Transform(GeomRotate(GeomResize(ST_Translate(a_g, (b_x-a_x) * (?), (b_y-a_y) * (?)), ?), ?), 4326) AS geom\n"
                + "FROM cluster \n"
                + "INNER JOIN (SELECT dataset_a_geometries.subject AS a_s,\n"
                + "		   dataset_b_geometries.subject AS b_s,\n"
                + "		   dataset_a_geometries.geom AS a_g,\n"
