...
;       // 
    ...
    map.addLayer(bboxLayer);
    var lays = [vectorsA, vectorsB, vectorsLinks, bboxLayer];
  ...
  map.setLayerIndex(bboxLayer,5);
        ...
        linkFeature.attributes = {'la': polygonFeature,  'a': 'ludacris', 'lb': polygonFeatureW};
        var links = [];
        links.push(linkFeature);
        polygonFeature.attributes = {'a': 'tomaras', 'links': links};
        polygonFeatureW.attributes = {'a': 'tomaras2', 'links': links};
        //vectorsLinks.addFeatures([linkFeature]);
...
function expandConnectionPanel() {
    hideAllPanels();
    if ( (lastClickedMenu != null) && (lastClickedMenu != $("#connectionMenu") ) ) {
        lastClickedMenu.data("currentlyOpened", false);
    }
    lastClickedMenu = $("#connectionMenu");
...
" <table class=\"table\" border=1 id=\"fusionTable\" style=\"width: 100%;>\n"+
" <tr>\n"+
...
function addUnlinkedMapDataJson(jsonentitiess) {
    $.each(jsonentitiess.entitiesA, function(index, element) {                
        ...
        polygonFeatureA.attributes = {'a': element.sub};
        polygonFeatureA.subject = element.sub;
        polygonFeatureA.style = unlinkedEntityStyleA;
    ...
    $.each(jsonentitiess.entitiesB, function(index, element) {             
        ...
        polygonFeatureB.attributes = {'a': element.sub};
        polygonFeatureB.subject = element.sub;
        polygonFeatureB.style = unlinkedEntityStyleB;
